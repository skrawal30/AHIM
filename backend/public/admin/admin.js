(() => {
  ...
  const resolvingCountries = new Set();

  async function browserGeoForIp(ip){
    ...
    for(const url of urls){
      try{
        ...
        if(d && !d.error){
          const code=String(d.country_code||d.countryCode||d.country_code_iso2||d.country||"").trim().toUpperCase();
  ...
  async function resolveUnknownCountries(rows){
    const candidates = rows.filter(s =>
      s && Number(s.SubmissionId) > 0 && s.ClientIP &&
      !/^[A-Z]{2}$/.test(String(s.CountryCode||"").toUpperCase()) &&
      ["","unknown","pending","local / unknown"].includes(String(s.Country||"").trim().toLowerCase())
    ).slice(0,10);

    await Promise.all(candidates.map(async s => {
      const id=Number(s.SubmissionId);
      if(resolvingCountries.has(id)) return;
      resolvingCountries.add(id);
      try{
        // First try the server-side resolver. If Railway cannot reach a
        // GeoIP service, resolve the exact stored customer IP from the
        // authenticated admin browser instead.
        let code=null;
        try{
          const r=await fetch(`/api/submissions/${encodeURIComponent(id)}/resolve-country`,{method:"POST",headers:auth()});
          if(r.status===401){logout();return;}
          if(r.ok){
            const d=await r.json();
            if(d.success && /^[A-Z]{2}$/.test(String(d.countryCode||"").toUpperCase())){
              s.Country=d.country||s.Country||"Unknown";
              s.CountryCode=String(d.countryCode).toUpperCase();
              render();
              return;
            }
          }
        ...
        code=await browserGeoForIp(s.ClientIP);
        if(!code)return;

        const r=await fetch(`/api/submissions/${encodeURIComponent(id)}/resolve-country`,{
          method:"POST",
        ...
        if(d.success){
          s.Country=d.country||s.Country||"Unknown";
          s.CountryCode=String(d.countryCode||code).toUpperCase();
          render();
        }
      }catch(err){console.warn("Country resolution failed:",err.message);}
      finally{resolvingCountries.delete(id);}
    }));
  }
  ...
  async function load({silent=false}={}){
    ...
    try{
      ...
      resolveUnknownCountries(allSubmissions).catch(err=>console.warn("Country background refresh:",err.message));
